    ...
    <Card>
      ...
      <Tabs
        ...
        extra={
          <Input.Search
            style={{ width: '240px' }}
            placeholder={[`cardList.tab.${activeKey}.placeholder`]}
          />
        }
      >
        <Tabs.TabPane key="all" title={['cardList.tab.title.all']} />
        <Tabs.TabPane key="quality" title={['cardList.tab.title.quality']} />
        <Tabs.TabPane key="service" title={['cardList.tab.title.service']} />
        <Tabs.TabPane key="rules" title={['cardList.tab.title.rules']} />
      </Tabs>
      <div className={styles.container}>
        {activeKey === 'all' ? (
          Object.entries(data).map(([key, list]) => (
            <div key={key}>
              <Title heading={6}>{[`cardList.tab.title.${key}`]}</Title>
